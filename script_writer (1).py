...
def _call_groq(api_key: str, prompt: str, temperature: float = 0.9,
    ...
    payload = {
        ...
        # openai/gpt-oss-120b is a reasoning model: with low max_tokens it can
        # burn its whole budget on hidden reasoning and return nothing at all
        # (json_validate_failed with an empty failed_generation). Capping
        # reasoning effort keeps it from eating the token budget before it
        # ever emits the actual answer, and also cuts real TPM usage.
...
{{
  ...
  "tone": "one of: conversational, educational, energetic, dramatic, funny"
    ...
    raw = _call_groq(api_key, prompt, temperature=0.9, json_mode=True, max_tokens=700)
    try:
        return json.loads(raw)
    except json.JSONDecodeError:
        # Occasionally the model wraps the JSON in prose or fences despite
        # instructions. Try to salvage the {...} substring before giving up.
