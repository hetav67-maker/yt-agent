...
def get_trending_topics(limit: int = 10) -> list[dict]:
    ...
    for item in root.findall(".//item")[:limit]:
        ...
        # grab the first related news headline if present, for extra context
...
def pick_topic() -> dict:
    """Pick one trending topic to build a video around (randomized among top results
    so repeated runs don't all pick the #1 trend)."""
...
if __name__ == "__main__":
    ...
    if topic["news_snippet"]:
        print(f"Context: {topic['news_snippet']}")
